...
interface LiveOllamaModelOptions {
  modelId: string;
  baseUrl?: string;
...
interface LiveOllamaModelLease {
  baseUrl: string;
  modelId: string;
}

export function isOllamaLiveEnabled(flagName = "GEMMA_DESKTOP_RUN_CLI_SCENARIOS_LIVE"): boolean {
...
async function ensureModelAvailability(
  ...
  if (!inspection.installed) {
    throw new Error(`Ollama is not installed or not discoverable at ${options.baseUrl}.`);
  }
...
export async function withLoadedLiveOllamaModel<T>(
  ...
  if (runError) {
    throw toError(runError, `Live Ollama test failed for model "${options.modelId}".`);
  }
  if (cleanupError) {
    throw toError(cleanupError, `Live Ollama cleanup failed for model "${options.modelId}".`);
  }
