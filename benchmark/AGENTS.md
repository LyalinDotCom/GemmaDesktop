...
Keep model capability and runtime transport capability separate. First decide whether the specific weight should support the modality, then decide whether the selected runtime API can deliver that modality to the model. If a model family can handle a modality but the selected runtime API cannot deliver that modality to the model, do not run the case and do not put a provider transport error in the Results table. Record it as not applicable coverage so the report stays honest about what was and was not actually tested. If the weight itself is not tagged or expected to support the modality, say that instead of blaming the runtime transport.
