...
export function filterModels(picker: ModelPickerState): SessionModelInfo[] {
  ...
  return picker.models.filter((model) => [
    model.name,
    model.provider,
    model.provider === 'lmstudio' ? model.displayName : undefined,
...
function modelDetail(model: SessionModelInfo): string {
  ...
  const display = model.provider === 'lmstudio' && model.displayName && model.displayName !== model.name ? model.displayName : '';
  ...
  const details = [
    ...
    model.provider === 'lmstudio' ? reasoningDetail(model.supportsReasoning) : '',
